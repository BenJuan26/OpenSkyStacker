...
std::vector<AdjoiningPixel> AdjoiningPixel::deblend(float base_step)
{
    ...
    while (pixels.size() > 0) {
        ...
        currentThreshold = peakPixel.getValue() - base_step * sqrt(peakPixel.getValue() / base_step + 1.0);
...
{
    Star star;
    star.setArea(pixels.size());

    star.setPeak(getPeakValue());
    float starValue = 0.0;
    for (ulong i = 0; i < pixels.size(); i++) {
        starValue += pixels.at(i).getValue();
    }
...
void AdjoiningPixel::extract(int x, int y)
{
    for (ulong i = 0 ; i < pixels.size() ; i++) {
        ...
        if (pixel.getX() == x  &&  pixel.getY() == y  &&  pixel.getValue() > currentThreshold) {
