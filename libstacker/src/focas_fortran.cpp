void hfti(float a[][MAX_MATCH], int mda, int m, int n, float b[][MAX_MATCH], int mdb, int nb, float tau, int &krank, float rnorm[], float h[], float g[], int ip[])
{
    ...
    for (int j = 0; j < ldiag; j++) {
        ...
        h12(1, j, j+1, m, a[j][1], 1, h[j], a[j+1][1], 1, mda, n-j);
        h12(2, j, j+1, m, a[j][1], 1, h[j], b, 1, mdb, nb);
    }
    int j;
    for (j = 0; j < ldiag; j++) {
        if (fabs(a[j][j]) > tau) {
    ...
    if (k != n) {
        for (int ii = 0; ii < k; ii++) {
            int i = kp1 - ii;
            // TODO: THESE NUMBERS ARE STILL 1-INDEXED
            h12(1, i, kp1, n, a[1][i], mda, &g[i], a, mda, 1, i-1);
        }
    }
    ...
    for (int jb = 0; jb < nb; jb++) {
        ...
        if (k != n) {
            ...
            for (int i = 0; i < k; i++)
                // TODO: THESE NUMBERS ARE STILL 1-INDEXED
                h12(2, i, kp1, n, a(i,1), mda, g[i], b[jb][1], 1, mdb, 1);
        }
